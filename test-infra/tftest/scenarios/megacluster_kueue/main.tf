...
locals {
  node_pool_parameters = merge(
    {
      for i in range(%TFTEST_NODE_POOL_COUNT%) : format("p%02d", i+1) => %TFTEST_NODE_POOL_SIZE%
    }
  )
}

locals {
  node_pool_parameters_cp = merge(
    {
      for i in range(%TFTEST_NODE_POOL_COUNT_COMPACT%) : format("pc%02d", i+1) => {
        size = %TFTEST_NODE_POOL_SIZE_COMPACT%
        zone = %TFTEST_NODE_LOCATIONS%[i % length(%TFTEST_NODE_LOCATIONS%)]
      }
    }
  )
}
...
resource "google_container_node_pool" "heapster-pool" {
  ...
  name = "heapster-pool"
  node_count = %TFTEST_HEAPSTER_NODE_COUNT%
  node_config {
    machine_type = "%TFTEST_HEAPSTER_NODE_POOL_MACHINE%"
  }
...
resource "google_container_node_pool" "%TFTEST_NODE_POOL_RESOURCE_NAME%cp" {
  for_each         = local.node_pool_parameters_cp
  cluster          = google_container_cluster.test_cluster.id
  name             = each.key
